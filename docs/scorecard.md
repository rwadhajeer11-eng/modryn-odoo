...
| 2 | Is custom booking as good as MODRYN's? | 🟡 **Yellow** | Both PRD paths worked end-to-end in **369 lines** as first built (the addon is ~460 now, after cancellation-aware slots and the waitlist form): dress-bound (`/book/dress/<id>`, binds the *variant* so size travels) and standalone (`/book`). Terms enforced server-side (rejected submission preserved input, created 0 rows). Slot collision re-checked at submit. DST-correct: 10:00 Israel stored as 07:00Z. **The “expensive 80%” has since been built, and the estimate was fair.** The fixed Sun–Thu 10:00–18:00 grid is now owner data (`modryn.opening.hours`, several windows a day), with blackout dates, per-window capacity enforced by a partial unique index on `(start, modryn_slot_seat)` rather than by Python, and the published rota capping what a day can sell. That took four features and about 850 lines, and Odoo contributed nothing domain-specific to any of it — the ORM, a migration hook and a partial index are the same tools any framework offers. **Still missing: per-appointment-type duration, phone OTP and deposits.** Duration is the one that stays hard rather than merely unbuilt: interval overlap cannot be expressed by a unique index at all, so it needs a `tstzrange` EXCLUDE constraint and `btree_gist`. Yellow stands — the engine is real now, but it was custom development end to end, which is the answer this row was asking about. |
