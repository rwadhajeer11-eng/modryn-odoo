...
python3 - "$TENANTS_JSON" "$SECRET" "$PORT" "$MANAGERS" "$STAFF" "${SLUGS[@]}" <<'PY'
import json, subprocess, sys

out, secret, port, managers, staff = sys.argv[1], sys.argv[2], sys.argv[3], int(sys.argv[4]), int(sys.argv[5])
slugs = sys.argv[6:]
...
for slug in slugs:
    ...
    tenants.append({
        'slug': slug,
        'baseUrl': 'http://%s.localtest.me:%s' % (slug, port),
        'staff': logins,
...
with open(out, 'w') as fh:
    json.dump({'tenants': tenants, 'loadtestSecret': secret}, fh, indent=2)
    fh.write('\n')
